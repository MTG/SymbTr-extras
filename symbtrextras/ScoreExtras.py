...
class ScoreExtras:
    _symbtr_mbid = json.load(open(  # load symbTr mbids
        os.path.join(os.path.dirname(os.path.abspath(__file__)),
                     '..', '..', 'symbTr_mbid.json'), 'r'))
    _iconv_map = {'utf-16le': 'UTF-16', 'Little-endian UTF-16 Unicode': 'UTF-16',
    ...
    def change_to_unix_linebreak(score_file):
        ...
        try:  # linux
            ...
            subprocess.check_call("sed -e 's/\r$//' " + score_file + " > tmp.txt " +
                                  "&& mv -f tmp.txt " + score_file, shell=True)
    ...
    def change_encoding_to_utf8(cls, score_file):
        try:  # unix
            ...
            if not any(curr_charset in charset for charset in ['utf-8', 'us-ascii']):
                ...
                commandstr = ("iconv -f " + cls._iconv_map[curr_charset] + " -t UTF-8 " + score_file + " > tmp.txt "
